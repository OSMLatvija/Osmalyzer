    ...
    public static OsmData Apply(OsmData data, List<SuggestedAction> changes, bool temporary)
    {
        ...
        foreach (SuggestedAction change in changes)
        {
            switch (change)
            {
                ...
                {
                    OsmElement osmElement = data.GetElementById(setValue.ElementType, setValue.Id);
                    osmElement.SetValue(setValue.Key, setValue.Value);
                    break;
                }

                case OsmRemoveKeySuggestedAction removeKey:
                {
                    OsmElement osmElement = data.GetElementById(removeKey.ElementType, removeKey.Id);
                    osmElement.RemoveTag(removeKey.Key);
                    break;
                }

                case OsmChangeKeySuggestedAction changeKey:
                {
                    OsmElement osmElement = data.GetElementById(changeKey.ElementType, changeKey.Id);
                    osmElement.RemoveTag(changeKey.OldKey);
                    osmElement.SetValue(changeKey.NewKey, changeKey.Value);
                    break;
                }
